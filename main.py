...
__author__ = "Mugilan Ganesan, CallMeShepard (Fork)"
...
def read(file_name):
    ...
    self_path = re.compile(r'(.+)main.py')
    ...
    processed_lines.append("Початок")
...
def translation(lines,font_data):
    ...
    input_re = re.compile(r'^INPUT|Input|input|CIN|cin\s(.+)')
    output_re = re.compile(r'^OUTPUT|COUT|cout|PRINT|print|printf\s(.+)')
    ...
    for i in range(0,len(lines)):
        ...
        if line == "Початок" or line == "Кiнець":
...
def drawer(chart_code,max_branch,max_y,layer_height,branch_width,font_data):
    ...
    def Flowline(role,position,height,chart_code,width):
        ...
        if role == 'n':
            ...
            draw.text(( (if_axis - width/2 + even_axis)/2 - block_gap/2,start-2*font_size), "Так", fill='black',font=font)
            draw.text(( (if_axis + width/2 + odd_axis)/2 - block_gap/3,start-2*font_size), "Нi", fill='black',font=font)
...
@click.option('--font', default=r"./fonts/timesnewromanpsmt.ttf", help="The font's path")
